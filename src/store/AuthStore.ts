...
const AUTH_STORAGE_KEY = 'crowdsourcing_auth';
const SESSION_EXPIRY_KEY = 'crowdsourcing_session_expiry';
const SESSION_DURATION = 24 * 60 * 60 * 1000;

const AUTH_CREDENTIALS: Record<Exclude<UserRole, 'viewer'>, string> = { contributor: 'contributor2026-crowd', admin: 'admin2026-crowd' };
...
export class AuthStore {
  ...
  get isContributor(): boolean { return this._user.role === 'contributor' || this._user.role === 'admin'; }
  ...
  canViewIdeas = (): boolean => this.permissions.canViewIdeas;
  canVote = (): boolean => this.permissions.canVote;
  canCreateIdeas = (): boolean => this.permissions.canCreateIdeas;
  canComment = (): boolean => this.permissions.canComment;
  canManageIdeas = (): boolean => this.permissions.canManageIdeas;
