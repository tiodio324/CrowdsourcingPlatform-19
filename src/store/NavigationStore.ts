...
export class NavigationStore {
  ...
  get navigationItems(): PageConfig[] { return Object.values(PAGES_CONFIG).filter(p => { if (!p.showInNav) return false; if (p.requiresAuth && !authStore.isAuthenticated) return false; if (p.requiredRole === 'admin' && !authStore.isAdmin) return false; if (p.requiredRole === 'contributor' && !authStore.isContributor) return false; return true; }); }
